...
public class PostRepo {
    ...
    //    /**
    //     * A repository interfész használatbavétele<br>
    //     * Impelemtálni nem kell, mert azt a JPA elvégzi helyettünk<br>
    //     */
    //    public PostRepo() {
    //    }
     ...
     * @param muvelet a DataBase  művelet (ENUM)törlés = -1; update = 0; insert == 1<br>
     ...
     */
    public Post merge(Post post, DBMUVELET muvelet) throws OwnException{                        
        //ez itt a hibakezlő IF blokk:
        // 1, ha törlés, vagy update lenne:
        if(muvelet.getLevel() <= 0){
            // ha nincs mire hivatkozni, gond van...
            // vagy ha van ugyan ID, csak éppen nem található ilyen, akkor is gond lesz!
            if((post.getId() == null) || ((post.getId() != null) && (this.findById(post.getId()) == null))){       
                throw new OwnException("JQL-MERGE-" + muvelet.name() + " vegrehajtasashoz nincs, vagy nincs ilyen vegrehajthato ID !");
            }
        }else{
            // 2, ha insert lenne:
            ...
            if(post.getId() != null){ 
                ...
                if(this.findById(post.getId()) != null){
                    throw new OwnException("JQL-MERGE-" + muvelet.name() + " vegrehajtasasahoz nem adhat meg ID-t (csak H2-teszt esetben, vagy MySQL-UPDATE eseten)!");
                }
            }
        }
        ...
        Post postRet = null;
        System.out.print("JQL-MERGE-" + muvelet.name() + " ");
        // ha törölni kell vagy update-olni:
        if(muvelet.getLevel() <= 0){
            // ez nem kell, mert a saját hibakezelő lekezeli: if(this.findById(post.getId()) != null){
            if(muvelet.getLevel() <= 0){
